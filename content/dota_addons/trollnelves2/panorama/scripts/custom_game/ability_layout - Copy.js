...
function EntityAbilityDataProvider(entityId) {
    ...
    this.ShowTooltip = function(element, data) {
		...
		var text;
		if(CustomNetTables.GetTableValue("upgrades", data.name)){	
			var name = CustomNetTables.GetTableValue("upgrades", data.name).unitname;
			var resources = CustomNetTables.GetTableValue("upgrade_costs", data.name) || CustomNetTables.GetTableValue("upgrade_costs", Entities.GetUnitName(data.caster) + "." + name) ;
			var requirements = CustomNetTables.GetTableValue("requirements", name);
			var gold_gain = CustomNetTables.GetTableValue("gold_gains",name);
			text = $.Localize("DOTA_Tooltip_ability_" + data.name, element) + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src='file://{images}/custom_game/gold_icon32.png'/>" + resources.gold_cost + "  <img src='file://{images}/custom_game/lumber_icon32.png'/>" + resources.lumber_cost +  "<br>" + $.Localize("DOTA_Tooltip_ability_" + data.name + "_Description", element);
			if(requirements){
				if(Object.keys(requirements).length > 0){
					text = text + "<br>Requirements:";
				}
				for(var key in requirements){
					text = text + "<br>" + $.Localize(key);
				}
				$.Msg(requirements);
			}
			if(gold_gain){
				text = text + "<br>Gives " + gold_gain.amount + "<img src='file://{images}/custom_game/gold_icon32.png'/> per second."
			}
			
		}else if(CustomNetTables.GetTableValue("items", data.name)){
			var item = CustomNetTables.GetTableValue("items", data.name);
			text = $.Localize("DOTA_Tooltip_ability_" + data.name, element);
			if ($.Localize("DOTA_Tooltip_ability_" + data.name + "_Description", element).length > 1){
				text = text + "<br>" + $.Localize("DOTA_Tooltip_ability_" + data.name + "_Description", element);
			}
			if(item.bonus_stats){
				text = text + "<br>" + $.Localize(item.bonus_stats,element) + ": " + item.bonus_value;
			}
			text = text + "<br><img src='file://{images}/custom_game/gold_icon32.png'/> " + item.gold_cost + "<br><img src='file://{images}/custom_game/lumber_icon32.png'/>" + item.lumber_cost;
		}else if(CustomNetTables.GetTableValue("lumberjackers", Entities.GetUnitName(data.caster))){
			var unit = CustomNetTables.GetTableValue("lumberjackers", Entities.GetUnitName(data.caster));
			text = $.Localize("DOTA_Tooltip_ability_" + data.name, element) + "<br>" + $.Localize("DOTA_Tooltip_ability_" + data.name + "_Description", element) + "<br>Lumber interval: " + unit.lumber_interval + "<br>Lumber amount:" + unit.amount;			
		}else if(data.name.indexOf("train_") > -1){
			var resources = CustomNetTables.GetTableValue("train", data.name);
			text = $.Localize("DOTA_Tooltip_ability_" + data.name, element) + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src='file://{images}/custom_game/gold_icon32.png'/>" + resources.gold_cost + "  <img src='file://{images}/custom_game/lumber_icon32.png'/>" + resources.lumber_cost;
			if (resources.food_cost > 0){
				text = text + "&nbsp;&nbsp;<img src='file://{images}/custom_game/cheese_icon32.png'/>" + resources.food_cost
			}
			text = text + "<br>" + $.Localize("DOTA_Tooltip_ability_" + data.name + "_Description", element)
			if(CustomNetTables.GetTableValue("lumberjackers", data.name.substr(6))){
				var lumberjack = CustomNetTables.GetTableValue("lumberjackers", data.name.substr(6));
				text = text + "<br>" + "Lumber interval: " + lumberjack.lumber_interval + "<br>Lumber amount: " + lumberjack.amount
			}
		}else{
			text = $.Localize("DOTA_Tooltip_ability_" + data.name, element) + "<br>" + $.Localize("DOTA_Tooltip_ability_" + data.name + "_Description", element);
		}
		$.DispatchEvent("DOTAShowTextTooltipStyled", element, text,"Tooltip");
    }
...
function UpdateHpReg(localHero){
	$("#HpRegText").text = "+" + parseFloat(Entities.GetHealthThinkRegen(localHero)).toFixed(2);
}
...
function UpdateMP(localHero){
	if(Entities.GetMaxMana(localHero) != 0){
		...
		$("#HealthBar").style.margin = "-10px 0px 0px 0px";
	}else{
		$("#HealthBar").style.margin = "10px 0px 0px 0px";
...
function UpdateArmor(localHero){
	$("#ArmorText").text = Entities.GetBonusPhysicalArmor(localHero) + Entities.GetPhysicalArmorValue(localHero);
	CurrentArmor = Entities.GetBonusPhysicalArmor(localHero) + Entities.GetPhysicalArmorValue(localHero);
}
